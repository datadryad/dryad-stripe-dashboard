...
export const setCustomStatus = async (invoice_id, status) => {
  let invoice = await Stripe.invoices.update(invoice_id, {
    metadata: {
...
export const reportNotPermitted = (user, permission) => {
  ...
  if (!perms.has(rep_permission) || perms.get(rep_permission) === "false")
    return true;
...
export const getAmountAndCountData = async (start_time, end_time) => {
  ...
  let total_amount = 0,
    total_count = 0,
    current_dataset = { data: [], has_more: true };
  ...
  while (current_dataset.has_more) {
    ...
    if (count)
      listObject["starting_after"] = current_dataset.data[count - 1].id;
...
export const initiateRestore = async () => {
  ...
  const latest_local_record = await Invoice.findOne(
    {},
    {},
    { sort: { created: -1 } }
  );
  ...
  if (latest_local_record) {
    ...
    console.log(
      "Latest saved Invoice record found is : ",
      latest_local_record_object._id
    );
    console.log(
      "Looking for any Invoice records created after the latest local invoice..."
    );
    ...
    try {
      ...
      console.log(
        "Couldn't find latest saved Invoice from local db in Stripe account, deleting all local records and fetching all Invoices from Stripe to avoid any discrepancies."
      );
    ...
    fetching_all = true;
    console.log(
      "No saved Invoice records found locally. Initiating restore..."
    );
  ...
  while (current_dataset.has_more) {
    ...
    if (current_dataset.data.length)
      listObject[fetching_all ? "starting_after" : "ending_before"] =
        current_dataset.data[current_dataset.data.length - 1].id;
    ...
    console.log(
      `[ ${ordinal(inv_index++)} Fetch ] Updating local database with ${
        current_dataset.data.length
      } new Invoice records...`
    );
  ...
  console.log(
    "Finished restore! Local database bas been updated with a total of " +
      inv_count +
      " new Invoice records."
  );
  return;
