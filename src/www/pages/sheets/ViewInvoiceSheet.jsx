        ...
        {invoice.id ? (
          <>
            <div>
              ...
              <h3>
                {invoice.number} for {getSymbolFromCurrency(invoice.currency) + invoice.amount_due}{" "}
              </h3>
            ...
            <div className="envelope-sub">
              ...
              <div className="row">
                <div>Charge Date : </div>
                <div>
                  <Tag>{hdate.prettyPrint(getDateObject(invoice.created), { showTime: true })}</Tag>
                </div>
              </div>
              <div className="row" style={{ marginTop: "1.2em", display: "flex", justifyContent: "flex-end", marginRight: "0.2em" }}>
                <Button size="huge" type="primary" danger icon={<RollbackOutlined />}>
                  Refund
                </Button>
              </div>
            </div>
        ...
        <div className="sheet no-margin invoice-container">
          <Row gutter={16}>
            ...
            <Col span={8}>
              ...
              <div className="bulky-divider"></div>
              <h4>
                Invoice total : {getSymbolFromCurrency(invoice.currency)}
                {Math.round(invoice.amount_due / 100)}
              </h4>
              <h4>
                Status : <StatusTag status={invoice.status} />
              </h4>
              {invoice.metadata?.custom_status === "voucher" || invoice.metadata?.custom_status === "waiver" ? (
                <h4>
                  Discount : ${invoice.metadata?.custom_status === "voucher" ? invoice.metadata?.voucher_amount : invoice.metadata?.waiver_amount} <StatusTag status={invoice.metadata.custom_status} />
                </h4>
              ) : (
                <></>
              )}
